...
namespace Characters
{
    public class FreelookState : CharacterStateBase
    {
        ...
        private readonly LayerMask buildingLayer = 1 << 6;
        ...
        public override void OnEnter()
        {
            ...
            _Owner._CM_VirtualCamera_Body.CameraDistance = _Owner._CameraControlConfig.MaxDistance;

            bodyPosition = _Owner._BodyTransform.position;
            //moveMult = 0.5f + (_Owner._CM_VirtualCamera_Body.CameraDistance - 50f) / 100f;
            moveMult = 1f + (_Owner._CM_VirtualCamera_Body.CameraDistance - _Owner._CameraControlConfig.MinDistance)
                / (_Owner._CameraControlConfig.MaxDistance - _Owner._CameraControlConfig.MinDistance);
        }
        ...
        public override void Update()
        {
            ...
            {
                if (Physics.Raycast(_Owner._Camera.ScreenPointToRay(Input.mousePosition), out RaycastHit raycastHit, 1000f, buildingLayer))
            ...
            */
            ...
            if (Input.GetMouseButton(0))
            {
                ...
                _Owner._BodyTransform.position += moveForward * _Owner._CameraControlConfig.MoveSpeed * moveMult;

                _Owner._BodyTransform.position = new Vector3(
                    Mathf.Clamp(_Owner._BodyTransform.position.x, _Owner._CameraControlConfig.AreaBottomLeftPosition.x, _Owner._CameraControlConfig.AreaTopRightPosition.x),
                    _Owner._BodyTransform.position.y,
                    Mathf.Clamp(_Owner._BodyTransform.position.z, _Owner._CameraControlConfig.AreaBottomLeftPosition.z, _Owner._CameraControlConfig.AreaTopRightPosition.z));
            }
            else if (Input.GetMouseButton(1))
            {
                ...
                temp.x = Mathf.Clamp(temp.x, _Owner._CameraControlConfig.MinAngle, _Owner._CameraControlConfig.MaxAngle);
                _Owner._HeadTransform.localRotation = Quaternion.Euler(Vector3.Scale(temp, new Vector3(1f, 1f, 0f)));
            }

            //滚动鼠标滚轮时改变相机距离以及移动速度倍率
            if (mouse_Wheel != 0f)
            {
                ...
                //moveMult = 0.5f + (_Owner._CM_VirtualCamera_Body.CameraDistance - 50f) / 100f;
                _Owner._CM_VirtualCamera_Body.CameraDistance -= mouse_Wheel * _Owner._CameraControlConfig.scaleSpeed;
                _Owner._CM_VirtualCamera_Body.CameraDistance =
                    Mathf.Clamp(_Owner._CM_VirtualCamera_Body.CameraDistance, _Owner._CameraControlConfig.MinDistance, _Owner._CameraControlConfig.MaxDistance);
            }

            moveMult = 0.5f + (_Owner._CM_VirtualCamera_Body.CameraDistance - _Owner._CameraControlConfig.MinDistance)
                / (_Owner._CameraControlConfig.MaxDistance - _Owner._CameraControlConfig.MinDistance);
        }
